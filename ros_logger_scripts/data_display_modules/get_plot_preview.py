...
from ros_logger_scripts.data_display_modules import preview_plot_update_thread, get_data_from_msg
...
class PlotPreviewWidget(QWidget):
    ...
    # TODO: запускать его в отдельном потоке
    def multiply_update_plot(self, axis_description, plot_title, plot_list, parsed_topic_w_msgs_dict):
        """
        Updates plot fields and fill variables that stores plot's data
        ...
        for one_plot_data in plot_list:
            print('one plot data: ', one_plot_data[0])
            # try to unwrap axis data
            if one_plot_data[0]:
                x_axis_data = get_data_from_msg.get_data(one_plot_data[0]['field data'], parsed_topic_w_msgs_dict)
            else:
                x_axis_data = None
            if one_plot_data[1]:
                y_axis_data = get_data_from_msg.get_data(one_plot_data[1]['field data'], parsed_topic_w_msgs_dict)
            else:
                y_axis_data = None

            print('x data:', x_axis_data)
            print('y data : ', y_axis_data)

            # if specific x and y data doesn't declare - display at x axis timestamps, at y axis data from message field
            if x_axis_data is None:
                x_axis = list()
                y_axis = list()
                for timestamp, msg in y_axis_data.items():
                    x_axis.append(timestamp)
                    y_axis.append(msg)

            elif y_axis_data is None:
                x_axis = list()
                y_axis = list()
                for timestamp, msg in x_axis_data.items():
                    x_axis.append(timestamp)
                    y_axis.append(msg)
